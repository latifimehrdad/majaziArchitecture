package land.majazi.latifiarchitecure.views.customs.wave;
...
public class LatifiWaveProgressView extends View {
    /*類型常數*/
    public enum Shape {
    ...
    public LatifiWaveProgressView(Context context) {
        this(context, null);
    }

    public LatifiWaveProgressView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public LatifiWaveProgressView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        /*取得xml參數*/
        TypedArray attributes = context.getTheme().obtainStyledAttributes(attrs, R.styleable.LatifiWaveProgressView, defStyleAttr, 0);

        /*設定xml參數*/
        mFrontWaveColor = attributes.getColor(R.styleable.LatifiWaveProgressView_frontColor, DEFAULT_FRONT_WAVE_COLOR);
        mBehindWaveColor = attributes.getColor(R.styleable.LatifiWaveProgressView_behideColor, DEFAULT_BEHIND_WAVE_COLOR);
        mBorderColor = attributes.getColor(R.styleable.LatifiWaveProgressView_borderColor, DEFAULT_BORDER_COLOR);
        mTextColor = attributes.getColor(R.styleable.LatifiWaveProgressView_textColor, DEFAULT_TEXT_COLOR);
        mProgress = attributes.getInt(R.styleable.LatifiWaveProgressView_progress, DEFAULT_PROGRESS);
        mMax = attributes.getInt(R.styleable.LatifiWaveProgressView_max, DEFAULT_MAX);
        mBorderWidth = attributes.getDimension(R.styleable.LatifiWaveProgressView_borderWidthSize, DEFAULT_BORDER_WIDTH);
        mStrong = attributes.getInt(R.styleable.LatifiWaveProgressView_strong, DEFAULT_STRONG);
        mShape = Shape.fromValue(attributes.getInt(R.styleable.LatifiWaveProgressView_shapeType, 1));
        mShapePadding = attributes.getDimension(R.styleable.LatifiWaveProgressView_shapePadding, DEFAULT_PADDING);
        isAnimation = attributes.getBoolean(R.styleable.LatifiWaveProgressView_animatorEnable, DEFAULT_ENABLE_ANIMATION);
        isHideText = attributes.getBoolean(R.styleable.LatifiWaveProgressView_textHidden, DEFAULT_HIDE_TEXT);
        charMark = attributes.getString(R.styleable.LatifiWaveProgressView_charMark);
        if (charMark == null || charMark.equalsIgnoreCase("null"))
            charMark = "";
    ...
    public void restartAnimation() {
        ...
        thread = new HandlerThread("YPWaveView_" + hashCode());
        thread.start();
        animHandler = new Handler(thread.getLooper());
        uiHandler = new UIHandler(new WeakReference<View>(this));
        Message message = Message.obtain(uiHandler);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (!isHideText) {
            ...
            String text = String.format(Locale.US, "%.0f", percent) +  charMark;
